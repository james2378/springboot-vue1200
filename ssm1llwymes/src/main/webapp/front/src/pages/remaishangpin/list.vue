		  ...
		  <div v-for="(item, index) in dataList" :key="index" @click.stop="toDetail(item)" :style='{"margin":"20px 2% 10px 0","overflow":"hidden","background":"#fff","width":"31.3%","minWidth":"320px","position":"relative","height":"280px"}' class="item">
			...
			<div class="content">
		      <div class="info" :style='{"width":"50%","padding":"0 0 0 10px","flexWrap":"wrap","flex":"1","display":"flex","height":"100%"}'>
				...
				<div :style='{"overflow":"hidden","color":"#fff","display":"-webkit-box","width":"100%","lineHeight":"1.5","fontSize":"14px","-webkit-box-orient":"vertical","-webkit-line-clamp":"3","order":"1"}' class="name ">宠物店名称:{{item.shangjiamingcheng}}</div>
    ...
    created() {
		...
		this.$http.get('option/pinpai/pinpai').then(res => {
			if (res.data.code == 0) {
				...
				this.selectOptionsList.push({name:'品种',list:this.pinpaiOptions,tableName: 'pinpai',check: -1})
			}
		});
      this.getFenlei();
      this.getList(1, '全部');
